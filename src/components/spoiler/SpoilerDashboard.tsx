...
export default function SpoilerDashboard({
  ...
  const [expiringItems, setExpiringItems] =
    useState<PantryItem[]>(initialExpiringItems);
  const expiringNames = expiringItems.map((item) => item.name);

  // Red-tier items: expires today or tomorrow (0–1 days). Used for scoring.
  ...
  // score = (redMatchCount × 10) + (totalMatched × 2) − (missedCount × 5)
  ...
  const allItemNames = useMemo(() => {
    ...
    const rest = items
      .filter((i) => !expiringSet.has(i.name))
      .map((i) => i.name);
        ...
        return (
          <div className="flex flex-col gap-6">
            ...
            {expiringItems.length > 0 && (
              <ExpiryAlertStrip items={expiringItems} />
            )}

            {/* Tonight's Hero — best recipe for red-tier items */}
            {heroRecipe && (
              <TonightsHeroCard recipe={heroRecipe} redItems={redItems} />
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-[#111111] border border-white/[0.08] rounded-[10px] p-4">
                  <h4 className="font-playfair text-sm font-semibold text-white mb-3">
                    💡 Tips
                  </h4>
                  <ul className="flex flex-col gap-2">
                    {[
                      ...
                      <li
                        key={i}
                        className="font-crimson text-sm text-white/50 flex gap-2"
                      >
            ...
            {recipesLoading || items.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-4 bg-[#111111] border border-white/[0.08] border-dashed rounded-[10px]">
                ...
                <div className="text-center">
                  <p className="font-playfair text-white/60 text-xl">
                    No expiring items
                  </p>
                  <p className="font-crimson text-white/30 text-base mt-1">
                    Track items in your pantry to discover recipes
                  </p>
                </div>
              </div>
            )}
        ...
        return (
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="font-crimson text-white/50">
                Track your food waste savings over time
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {[
                {
                  label: "Total Saved",
                  value: `$${savings.totalSaved.toFixed(2)}`,
                  color: "text-[#0D9488]",
                  icon: "💰",
                },
                {
                  label: "This Month",
                  value: `$${savings.monthSaved.toFixed(2)}`,
                  color: "text-[#0D9488]",
                  icon: "📅",
                },
                {
                  label: "Items Used",
                  value: String(savings.totalUsed),
                  color: "text-white",
                  icon: "✅",
                },
                {
                  label: "Items Wasted",
                  value: String(savings.totalWasted),
                  color: "text-red-400",
                  icon: "🗑️",
                },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="bg-[#111111] border border-white/[0.08] rounded-[10px] p-5"
                >
                  ...
                  <p
                    className={`font-playfair text-3xl font-bold ${stat.color} mt-1`}
                  >
                    {stat.value}
                  </p>
                </div>
